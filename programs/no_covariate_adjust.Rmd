...
p_total
ggsave("../figures/no_covariate_adjust/violine_total_v1.jpeg", width = 6.25, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/violine_total_v1.pdf", width = 6.25, height = 5)
```
...
p_total = ggviolin(df_v2, x = "wheezing", y = "total", color = "wheezing",
  ...
  scale_color_npg(name = NULL)
p_total
ggsave("../figures/no_covariate_adjust/violine_total_v2.jpeg", width = 6.25, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/violine_total_v2.pdf", width = 6.25, height = 5)
```
...
p_box
ggsave("../figures/no_covariate_adjust/box_plot_time_change.jpeg", width = 10, height = 6, dpi = 100)
ggsave("../figures/no_covariate_adjust/box_plot_time_change.pdf", width = 10, height = 6)
```
...
p_box_alpha
ggsave("../figures/no_covariate_adjust/box_plot_alpha.jpeg", width = 6.25, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/box_plot_alpha.pdf", width = 6.25, height = 5)
```
...
p_box = ggarrange(bxp_list_v1[[1]], bxp_list_v1[[2]], bxp_list_v1[[3]], 
                  ...
                  common.legend = TRUE, legend = "bottom")
p_box
ggsave("../figures/no_covariate_adjust/box_plot.jpeg", width = 12, height = 10, dpi = 100)
ggsave("../figures/no_covariate_adjust/box_plot.pdf", width = 12, height = 10)
```
...
p_clme1
ggsave("../figures/no_covariate_adjust/clme_plot_v1_increase.jpeg", width = 12, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/clme_plot_v1_increase.pdf", width = 12, height = 5)
```
...
p_clme2 = ggarrange(fig1, fig2, fig3, fig4, fig5, fig6, 
                    ncol = 6, common.legend = TRUE, legend = "bottom")
p_clme2
ggsave("../figures/no_covariate_adjust/clme_plot_v1_decrease.jpeg", width = 12, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/clme_plot_v1_decrease.pdf", width = 12, height = 5)
```
...
p_clme1 = ggarrange(fig1, fig2, fig3, fig4, fig5, fig6, 
                    ncol = 6, common.legend = TRUE, legend = "bottom")
p_clme1
ggsave("../figures/no_covariate_adjust/clme_plot_v2_increase.jpeg", width = 12, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/clme_plot_v2_increase.pdf", width = 12, height = 5)
```
...
p_clme2 = ggarrange(fig1, fig2, fig3, fig4, fig5, fig6, 
                    ncol = 6, common.legend = TRUE, legend = "bottom")
p_clme2
ggsave("../figures/no_covariate_adjust/clme_plot_v2_decrease.jpeg", width = 12, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/clme_plot_v2_decrease.pdf", width = 12, height = 5)
```

# 5. Correlations {.tabset}

1. SCFAs correlations by the outcome variable (any report of asthma or wheezing since birth).

2. Fisher's z-transformation was used to determine whether two Pearson correlation coefficients are the same or not. 

3. Only entries with p-value < 0.05 (correlations that were significantly different between "wheezing = No" and "wheezing = Yes") were shown.
...
corr1 = cor(df_scfa_v1 %>%
              filter(wheezing == "No") %>%
              dplyr::select(-wheezing)) 
corr1[is.na(corr1)] = 0

corr2 = cor(df_scfa_v1 %>%
              filter(wheezing == "Yes") %>%
              dplyr::select(-wheezing)) 
corr2[is.na(corr2)] = 0

mat_p_v1 = matrix(NA, nrow = length(scfa_list), ncol = length(scfa_list))
colnames(mat_p_v1) = scfa_list
rownames(mat_p_v1) = scfa_list
for (i in seq_along(scfa_list[-1])) {
  scfa1 = scfa_list[i]
  for (j in seq.int(from = i + 1, to = length(scfa_list))) {
    scfa2 = scfa_list[j]
    p_val = fisher_z(data = df_scfa_v1, var1 = scfa1, var2 = scfa2, group = "wheezing")
...
corr1_filter = corr1
corr1_filter[mat_p_v1 >= 0.05] = 0
diag(corr1_filter) = 1
df_corr1 = as.data.frame(corr1_filter) %>%
  rownames_to_column("var1") %>%
  pivot_longer(cols = -var1, names_to = "var2", values_to = "value")

corr2_filter = corr2
corr2_filter[mat_p_v1 >= 0.05] = 0
diag(corr2_filter) = 1
df_corr2 = as.data.frame(corr2_filter) %>%
  rownames_to_column("var1") %>%
  pivot_longer(cols = -var1, names_to = "var2", values_to = "value")
...
p_heat1 = df_corr1 %>%
  ...
  geom_text(aes(label = ifelse(value != 0, round(value, 2), "")), color = "black") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0,
                       name = NULL) +
  labs(x = NULL, y = NULL, title = "Wheezing = No") +
  theme_minimal() +
...
p_heat2 = df_corr2 %>%
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0,
                       name = NULL) +
  labs(x = NULL, y = NULL, title = "Wheezing = Yes") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
p_heat_v1 = ggarrange(p_heat1, p_heat2, common.legend = TRUE, legend = "right")
p_heat_v1
ggsave("../figures/no_covariate_adjust/heatmap_v1.jpeg", width = 10, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/heatmap_v1.pdf", width = 10, height = 5)
```
...
mat_p_v2 = matrix(NA, nrow = length(scfa_list), ncol = length(scfa_list))
...
for (i in seq_along(scfa_list[-1])) {
  ...
  for (j in seq.int(from = i + 1, to = length(scfa_list))) {
    scfa2 = scfa_list[j]
    p_val = fisher_z(data = df_scfa, var1 = scfa1, var2 = scfa2, group = "wheezing")
    if (is.na(p_val)) p_val = 0
    mat_p_v2[i, j] = p_val
  }
}
...
p_heat_v2 = ggarrange(p_heat1, p_heat2, common.legend = TRUE, legend = "right")
p_heat_v2
ggsave("../figures/no_covariate_adjust/heatmap_v2.jpeg", width = 10, height = 5, dpi = 100)
ggsave("../figures/no_covariate_adjust/heatmap_v2.pdf", width = 10, height = 5)
```
